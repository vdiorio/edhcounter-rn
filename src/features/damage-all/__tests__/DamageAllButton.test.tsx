...
describe('DamageAllButton', () => {
  ...
  it('tap reduces all opponents life by 1 and keeps caller unchanged', () => {
    ...
    expect(useGameStore.getState().players[0]!.lTotal).toBe(
      STARTING_LIFE_TOTAL,
    );
    expect(useGameStore.getState().players[1]!.lTotal).toBe(
      STARTING_LIFE_TOTAL - 1,
    );
    expect(useGameStore.getState().players[2]!.lTotal).toBe(
      STARTING_LIFE_TOTAL - 1,
    );
    expect(useGameStore.getState().players[3]!.lTotal).toBe(
      STARTING_LIFE_TOTAL - 1,
    );
  });
  ...
  it('holding for 1200ms triggers at least two heal ticks on opponents', () => {
    ...
    expect(useGameStore.getState().players[1]!.lTotal).toBeGreaterThanOrEqual(
      STARTING_LIFE_TOTAL + 2,
    );
    expect(useGameStore.getState().players[2]!.lTotal).toBeGreaterThanOrEqual(
      STARTING_LIFE_TOTAL + 2,
    );
    expect(useGameStore.getState().players[3]!.lTotal).toBeGreaterThanOrEqual(
      STARTING_LIFE_TOTAL + 2,
    );
  });
  ...
  it('animates the outline toward fast tap and hold rotations', () => {
    ...
    expect(withTimingSpy).toHaveBeenCalledWith(45, expect.any(Object));
    expect(withTimingSpy).toHaveBeenCalledWith(45, expect.any(Object));
